...
mid_test_files_abs = sorted(
    list(map(lambda x: os.path.join(mids_dir, x), mid_test_files))
)


rel_image_paths = ["face.bmp", "gun.bmp", "test.bmp"]

abs_image_paths = list(map(lambda s: os.path.join(script_dir, s), rel_image_paths))
    ...
    def check_for_eq(extremely_verbose=False):
        if y > 0:
            ...
            if image[x, y] == image[x, y - 1] and adjacencies_differ:
                ...
                eq_classes.append(
                    (
                        min(regions[x, y - 1], regions[x - 1, y]),
...
def set_initial_regions(image, region, verbose=False):
    ...
    for i in range(0, image.shape[0]):
        for j in range(0, image.shape[1]):
            if verbose:
                print(
                    ("Region [{}, {}]" " is starting at {}").format(i, j, region[i, j])
                )
    ...
    if len(eqs) < 2:
        ...
        for t in ts:
            if head == t:
                ...
                if verbose:
                    print(
                        ("  {} is being removed," " and {} appended.").format(
                            t, (head[0], t[1])
                        )
                    )
                tail.remove(t)
...
if __name__ == "__main__":
    ...
    for img in [os.path.join(script_dir, "gun.bmp")]:
        ...
        print("Single pixels filtered from gun.bmp:", size_filter(p_orig, p_filtered))
